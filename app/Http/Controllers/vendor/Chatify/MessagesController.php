...
class MessagesController extends Controller
{
    ...
    protected $messengerFallbackColor = '#673AB7';
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array(strtolower($file->getClientOriginalExtension()), $allowed)) {
                    ...
                    $error->status = 1;
                    $error->message = "Cette extension de fichier n'est pas autorisé!";
                }
            } else {
                $error->status = 1;
                $error->message = "La taille du fichier que vous essayer d'ajouter est trop élevée!";
            }
        }
    ...
    public function getContacts(Request $request)
    {
        ...
        if (count($usersList) > 0) {
            ...
            $contacts = '<p class="message-hint center-el"><span>Vous n\'avez aucune discssion</span></p>';
        }
    ...
    public function search(Request $request)
    {
        ...
        if($records->total() < 1){
            $getRecords = '<p class="message-hint center-el"><span>Aucun résultat.</span></p>';
        }
        // send the response
        return Response::json([
            'records' => $getRecords,
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        // send the response
        return Response::json([
            'shared' => count($shared) > 0 ? $sharedPhotos : '<p class="message-hint"><span>Aucun partage pour le moment</span></p>',
        ], 200);
    }
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                ...
                $msg = "La taille du fichier que vous essayez d'envoyer est trop élevée!";
                $error = 1;
            }
        }
